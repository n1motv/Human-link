...
def ajouter_conge_mensuel():
    ...
    curseur.execute("SELECT id, conge, dernier_mois_maj FROM users")
    employes = curseur.fetchall()
    
    for employe in employes:
        employe_id, conge, dernier_mois_maj = employe
        if dernier_mois_maj != mois_actuel:
            nouveau_solde_conge = conge + 2.5
            curseur.execute("""
                UPDATE users
                SET conge = ?, dernier_mois_maj = ?
                WHERE id = ?
